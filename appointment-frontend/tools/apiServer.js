...
server.post("/appointments/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.slug = createSlug(req.body.name); // Generate a slug for new appointments.
    next();
  }
});
...
function validateAppointment(appointment) {
  if (!appointment.name) return "Patient Full Name is required.";
  if (!appointment.dob) return "Date of Birth is required.";
  if (!appointment.gender) return "Gender is required.";
  if (!appointment.disease) return "Disease is required.";
  if (!appointment.address) return "Address is required.";
  if (!appointment.city) return "City is required.";
  if (!appointment.state) return "State is required.";
  if (!appointment.country) return "Country is required.";
  if (!appointment.zip) return "Zip is required.";
  if (!appointment.phone) return "Phone is required.";
  if (!appointment.time) return "Appointment time is required.";
  if (!appointment.doctor) return "Doctor is required.";
  if (!appointment.room) return "Room number is required.";
